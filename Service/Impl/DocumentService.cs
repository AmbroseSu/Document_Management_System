    ...
    public async Task<ResponseDto> GetDocumentDetailById(Guid documentId, Guid userId)
    {
        ...
        if (document.DocumentWorkflowStatuses.FirstOrDefault().Workflow.Scope == Scope.InComing)
        {
            ...
            try
            {
                ...
                string filePath = Path.Combine(_storagePath,"document",document.DocumentVersions.FirstOrDefault(x => x.IsFinalVersion).DocumentVersionId.ToString(), $"config{fileExtension}");
